...
class TestUserRepository:
    ...
    @pytest.mark.parametrize(
        "expectedData",
        [
            {"id": 5, "username": "john123", "password": "password123", "role": "Employee"},
            {"id": 6, "username": "harper123", "password": "password123", "role": "Employee"},
            {"id": 7, "username": "amara123", "password": "password123", "role": "Employee"}
        ]
    )
    def test_find_by_username_positive(self, expectedData, setUp):
        # Arrange
        ...
        #Act
        actualUser = setUp[3].find_by_username(expectedData["username"])
    ...
    @pytest.mark.parametrize(
        ...
        [
            {"id": 5, "username": "john123", "password": "password123", "role": "Employee"},
            {"id": 6, "username": "harper123", "password": "password123", "role": "Employee"},
            {"id": 7, "username": "", "password": "password123", "role": "Employee"}
        ]
    )
    def test_find_by_username_not_found(self, expectedData, setUp):
        # Arrange
        # setUp[0] = mock_db, setUp[1] = mock_conn,
        # setUp[2] = mock_cursor, setUp[3] = userRepo
        setUp[0].get_connection.return_value.__enter__.return_value = setUp[1]
    ...
    def test_find_by_id_positive(self, expectedData, setUp):
        ...
        setUp[2].fetchone.return_value = expectedData
        # Act
        actualUser = setUp[3].find_by_id(expectedData["id"])
        # Assert
        assert actualUser.id == expectedData['id']
        assert actualUser.username == expectedData['username']
        assert actualUser.password == expectedData['password']
    ...
    def test_find_by_id_not_found(self, invalidId, setUp):
        # Arrange
        # setUp[0] = mock_db, setUp[1] = mock_conn,
        # setUp[2] = mock_cursor, setUp[3] = userRepo
        setUp[0].get_connection.return_value.__enter__.return_value = setUp[1]
        ...
        setUp[2].fetchone.return_value = None
        # Act
        actualUser = setUp[3].find_by_id(invalidId)
    ...
    @pytest.mark.parametrize(
        "newUser",
        [
            User(id=None, username="abc123", password="password123", role="Employee"),
            User(id=None, username="testUser1", password="passwordTest1", role="Employee"),
            User(id=None, username="testUser2", password="passwordTest2", role="Employee")
        ]
    )
    def test_create_user_positive(self, newUser, setUp):
        # Arrange
        # setUp[0] = mock_db, setUp[1] = mock_conn,
        # setUp[2] = mock_cursor, setUp[3] = userRepo
